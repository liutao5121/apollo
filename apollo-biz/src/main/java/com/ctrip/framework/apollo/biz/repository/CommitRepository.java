...
public interface CommitRepository extends PagingAndSortingRepository<Commit, Long> {
  ...
  @Modifying
  @Query("update Commit set isDeleted = 1, deletedAt = ROUND(UNIX_TIMESTAMP(NOW())*1000), dataChangeLastModifiedBy = ?4 where appId=?1 and clusterName=?2 and namespaceName = ?3 and isDeleted = 0")
