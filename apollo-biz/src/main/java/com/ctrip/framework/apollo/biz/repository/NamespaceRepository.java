...
public interface NamespaceRepository extends PagingAndSortingRepository<Namespace, Long> {
  ...
  @Modifying
  @Query("update Namespace set isDeleted = 1, deletedAt = ROUND(UNIX_TIMESTAMP(NOW())*1000), dataChangeLastModifiedBy = ?3 where appId=?1 and clusterName=?2 and isDeleted = 0")
